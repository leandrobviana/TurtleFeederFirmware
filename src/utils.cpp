...
boolean inSummerTime(time_t time)
{
    ...
    { // March to October inclusive
        ...
        if (months == 3)
        { // Have we passed the last Sunday of March, 1am UT ?
            if (days >= 25)
        ...
        if (months == 10)
        { // Have we passed the last Sunday of October 1am UT ?
            if (days >= 25)
            { // It's getting interesting
                int dw = getDayOfWeek(days, months, years);
                // When is the next sunday ?
...
#ifdef useDST
            if (inSummerTime(lastKnownTime + (millis() - lastKnownTimeAT) / 1000))
            {                                                                                                      // We can't use "now()" instead here becouse it will make a loop
